...
from lib import train_test_split_madebymitsuhashi,decide_test_number
...
#sensitivity = 'nosensitivity/opticsexpress_select_3'
sensitivity = 'nosensitivity'
#sensitivity = 'nosensitivity/rate_max_opticsexpress'
from_frequency = 1.4
to_frequency = 1.6
frequency_list = [] #周波数を指定しない場合は空にして下さい。
inten_or_trans_or_reflect = 2 #0の時強度、1の時透過率、2の時反射率
#mainデータを読み込む。
last_type = 3 #使用する種類
...
add = 3 #フォルダを新しく追加した場合そのフォルダの数　1つの場合は1で行けるかな？？
#######
...
#         for j in range(1, last_num + 1):
#             try:
#                 x = allread(inten_or_trans_or_reflect, str(i) + thickness, i, j, last_type, last_num, from_frequency,
#                             to_frequency, frequency_list).Frequency_trans_reflect_is_TPG(
#                     date_dir + shielding_material + '/' + sensitivity + '/' + str(i) + '/' + str(k) + '/' + str(j) + '.txt',
#                     date_dir + shielding_material + '/' + sensitivity + '/' + str(i) + '/' + str(k) + '/' + 'ref.txt')
#
#                 file_name_list.append(j)
#
#                 if flag == 0:
